...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/php4g28oogo/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function group($tablesName,$columnName){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tokens['tablename'] == 'goumaikecheng'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'xuexiziliao'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'xuexikecheng'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'xuexijindu'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tokens['tablename'] == 'jifenduihuan'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
    ...
    public function value($tableName,$xColumnName,$yColumnName,$tjType){
        ...
        $where = " where 1 ";
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tableName == 'goumaikecheng'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'xuexiziliao'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'xuexikecheng'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'xuexijindu'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'jifenduihuan'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                }
            }
