...
class KechengxinxiController extends CommonController {
    ...
    public function autoSort2(){
		...
		$token = $this->token();
        $tokens = json_decode(base64_decode($token),true);
        $storeupSql = "select inteltype from storeup where userid = ".$tokens['id']." and tablename = 'kechengxinxi' order by addtime desc";
        ...
        if (count($storeupType) > 0) {
            $lists = "select * from kechengxinxi where kechengfenlei in (".implode(', ', $storeupType).") union all select * from kechengxinxi where kechengfenlei not in (".implode(', ', $storeupType).")";
        } else {
            $lists = "select * from kechengxinxi";
        }
    ...
    public function count() {
        ...
        $sql = "select count(*) as count from kechengxinxi where 1=1 ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
