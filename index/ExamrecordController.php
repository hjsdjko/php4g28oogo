...
class ExamrecordController extends CommonController {
    ...
    public function options(){
        ...
        if (isset($_REQUEST['paperid'])){
            $where .= " and `paperid` like '".$_REQUEST['paperid']."' ";
        }

        $sql = "select MAX(options) AS options,
        MAX(questionname) AS questionname,
        SUM(myscore) AS myscore,
        ROUND(
            CAST(SUM(CASE WHEN myscore > 0 THEN 1 ELSE 0 END) AS DECIMAL(10, 2)) / CAST(COUNT(myscore) AS DECIMAL(10, 2)),
            2
        ) AS accuracy,SUM(CASE WHEN myanswer LIKE '%A%' THEN 1 ELSE 0 END) AS anum,
        SUM(CASE WHEN myanswer LIKE '%B%' THEN 1 ELSE 0 END) AS bnum,
        SUM(CASE WHEN myanswer LIKE '%C%' THEN 1 ELSE 0 END) AS cnum,
        SUM(CASE WHEN myanswer LIKE '%D%' THEN 1 ELSE 0 END) AS dnum from examrecord ".$where." group by questionid";
        $result = table_sql($sql);
        if ($result->num_rows < 1){
            $numberCount = 1;
        }else{
            $numberCount = $result->num_rows;
        }
        $page_count = ceil($numberCount/$limt);//页数
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM examrecord ".$where." group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM examrecord ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM examrecord ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM examrecord ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM examrecord ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
