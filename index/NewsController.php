...
class NewsController extends CommonController {
    ...
    public function page(){
        ...
        $lists = "select * from `news` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function list(){
        ...
        $page_count = ceil($numberCount/$limt);//页数
        $startCount = ($page-1)*$limt;
        $lists = "select * from `news` ".$where." order by ".$sortorders." limit ".$startCount.",".$limt;
        $result = table_sql($lists);
        $arrayData = array();
        if ($result->num_rows > 0) {
            while ($datas = $result->fetch_assoc()){
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `news` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
    ...
    public function update(){
        ...
        $value = implode(',',$v);
        $sql = "UPDATE news SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function info($id=false){
        ...
        $this->andone('news',$id,$column='clicknum',$num=1);
        $clicktime = "update news set clicktime = '".date('Y-m-d H:i:s',time())."' where ".$where;
        table_sql($clicktime);
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM news ".$where." group by ".$xColumnName;
    ...
    public function valueMul(){
        ...
        foreach(explode(",", $yColumnName) as $item){
            $sql = "SELECT ".$xColumnName.",sum(".$item.") total FROM news ".$where." group by ".$xColumnName." LIMIT 10";
            if(urldecode($request[6]) == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$item.") total FROM news ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
            }
            if(urldecode($request[6]) == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$item.") total FROM news ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
            }
            if(urldecode($request[6]) == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$item.") total FROM news ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
            }
            $result = table_sql($sql);
    ...
    public function thumbsup($id=false){
        ...
        if ($type==1){
            $result = $this->andone('news',$id,$column = 'thumbsupnum',$num = 1);
        }else{
            $result = $this->andone('news',$id,$column = 'crazilynum',$num = 1);
        }
    ...
    public function autoSort(){
        ...
        if ($where) {
		    $lists = "select * from `news` where ".$where." order by ".$sort." desc limit 0,".$limt;
        }else{
		    $lists = "select * from `news` order by ".$sort." desc limit 0,".$limt;
        }
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
    ...
    public function autoSort2(){
		...
		$token = $this->token();
        $tokens = json_decode(base64_decode($token),true);
        $storeupSql = "select inteltype from storeup where userid = ".$tokens['id']." and tablename = 'news' order by addtime desc";
        $storeupInfo = table_sql($storeupSql);
        $storeupType = array();
        if ($storeupInfo->num_rows > 0) {
            while ($storeupRow = $storeupInfo->fetch_assoc()){
                array_push($storeupType,"'".$storeupRow['inteltype']."'");
            }
        }
        if (count($storeupType) > 0) {
            $lists = "select * from news where typename in (".implode(', ', $storeupType).") union all select * from news where typename not in (".implode(', ', $storeupType).")";
        } else {
            $lists = "select * from news";
        }
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM news ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
