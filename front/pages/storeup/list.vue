...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 4rpx","borderRadius":"16rpx","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.name" type="text" placeholder="名称" ></input>
				</view>
			...
			<view :style='{"width":"100%","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
				<view class="uni-product-list" :style='{"padding":"0px","margin":"40rpx 0 0","color":"#76c4d8","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
					<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","backgroundColor":"#fff","borderRadius":"60rpx","flexWrap":"wrap","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
						<view class="uni-product-title " :style='{"width":"100%","padding":"4rpx 20rpx","lineHeight":"48rpx","fontSize":"28rpx","color":"#333"}'>{{product.name}}</view>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"60rpx","display":"block","width":"100%","height":"300rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.picture)" :src="product.picture"></image>
						<image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"60rpx","display":"block","width":"100%","height":"300rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-else :src="baseUrl+product.picture"></image>
					</view>
				</view>
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('storeup','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('storeup','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
