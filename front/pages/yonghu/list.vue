...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 4rpx","borderRadius":"16rpx","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.yonghuzhanghao" type="text" placeholder="用户账号" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing01" :style='{"color":"#a5ccc4","left":"0px","textAlign":"center","display":"none","width":"80rpx","fontSize":"40rpx","lineHeight":"68rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.yonghuxingming" type="text" placeholder="用户姓名" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.touxiang)" :src="product.touxiang.split(',')[0]"></image>
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.touxiang?baseUrl+product.touxiang.split(',')[0]:''"></image>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('yonghu','修改')) || (!userid && isAuthFront('yonghu','修改'))" @tap.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('yonghu','删除')) || (!userid && isAuthFront('yonghu','删除'))" @tap.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('yonghu','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('yonghu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
