	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('kechengfenlei', this.id);
                this.detail = res.data;
				this.title = this.detail.kechengfenlei
            }
		},
		destroyed: function() {
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','kechengfenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','kechengfenlei');
			...
			async init(){
				...
				let res = await this.$api.info('kechengfenlei', this.id);
				this.detail = res.data;
