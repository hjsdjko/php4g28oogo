	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx 24rpx 80rpx","position":"relative","background":"#eefcff","height":"auto"}'>
		<form :style='{"padding":"24rpx 48rpx","boxShadow":"0 4rpx 12rpx #76c4d880","borderRadius":"60rpx","background":"#fff","display":"block","width":"100%","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 0px 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
                ...
                <xia-editor ref="editor" :style='{"border":"2rpx solid #76c4d8","minHeight":"400rpx","width":"100%","padding":"20rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjianglihuodong`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjianglihuodong`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjianglihuodong`, this.ruleForm);
						}else{
							await this.$api.add(`discussjianglihuodong`, this.ruleForm);
						}
						uni.setStorageSync('discussjianglihuodongCleanType',true);
						var res1 = await this.$api.info('jianglihuodong',this.ruleForm.refid)
						res1.data.discussnum++
						await this.$api.update('jianglihuodong',res1.data)
