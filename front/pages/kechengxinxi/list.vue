...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			...
			<view :style='{"width":"100%","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"padding":"20rpx 0 0","margin":"0","whiteSpace":"nowrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					<view :class='categoryName === item.kechengfenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.kechengfenlei)">{{item.kechengfenlei}}</view>
				</scroll-view>
			...
			<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.kechengfengmian)" :src="product.kechengfengmian.split(',')[0]"></image>
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.kechengfengmian?baseUrl+product.kechengfengmian.split(',')[0]:''"></image>
					...
					<view class="listmr" :style='{"width":"calc(100% - 180rpx)","padding":"0","margin":"0","height":"auto"}'>
						...
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.kechengfenlei}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kechengxinxi','修改')) || (!userid && isAuthFront('kechengxinxi','修改'))" @tap.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('kechengxinxi','删除')) || (!userid && isAuthFront('kechengxinxi','删除'))" @tap.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('kechengxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('kechengxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('kechengfenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('kechengfenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,kechengfenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.kechengfenlei = '%' + this.categoryName + '%'
				}
