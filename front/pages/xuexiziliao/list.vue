...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 4rpx","borderRadius":"16rpx","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.ziliaomingcheng" type="text" placeholder="资料名称" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.ziliaofengmian)" :src="product.ziliaofengmian.split(',')[0]"></image>
					<image :style='{"width":"160rpx","margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"100%","display":"block","height":"160rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.ziliaofengmian?baseUrl+product.ziliaofengmian.split(',')[0]:''"></image>

					<view class="listmr" :style='{"width":"calc(100% - 180rpx)","padding":"0","margin":"0","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.ziliaomingcheng}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xuexiziliao','修改')) || (!userid && isAuthFront('xuexiziliao','修改'))" @tap.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xuexiziliao','删除')) || (!userid && isAuthFront('xuexiziliao','删除'))" @tap.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('xuexiziliao','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"80rpx","borderRadius":"100%","background":"#76c4d8","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('xuexiziliao','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xuexiziliao`, params);
                } else {
                    res = await this.$api.list(`xuexiziliao`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xuexiziliao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xuexiziliao`, searchForm);
                } else {
                    res = await this.$api.list(`xuexiziliao`, searchForm);
                }
				// 如果是第一页数据置空
