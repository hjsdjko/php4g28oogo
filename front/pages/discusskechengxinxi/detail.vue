...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusskechengxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusskechengxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusskechengxinxi', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusskechengxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusskechengxinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discusskechengxinxi', this.id);
				this.detail = res.data;
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusskechengxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusskechengxinxi/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusskechengxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusskechengxinxi',that.detail)
							setTimeout(()=>{
