	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusskechengxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusskechengxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusskechengxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discusskechengxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discusskechengxinxiCleanType',true);
						var res1 = await this.$api.info('kechengxinxi',this.ruleForm.refid)
						res1.data.discussnum++
						await this.$api.update('kechengxinxi',res1.data)
