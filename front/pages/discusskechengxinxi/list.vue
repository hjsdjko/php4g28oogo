			...
			<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusskechengxinxi','修改')) || (!userid && isAuthFront('discusskechengxinxi','修改'))" @tap.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusskechengxinxi','删除')) || (!userid && isAuthFront('discusskechengxinxi','删除'))" @tap.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusskechengxinxi`, params);
                } else {
                    res = await this.$api.list(`discusskechengxinxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discusskechengxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusskechengxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`discusskechengxinxi`, searchForm);
                }
				// 如果是第一页数据置空
