<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">活动编号</label>
						<el-input v-model="searchForm.huodongbianhao" placeholder="活动编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">活动标题</label>
						<el-input v-model="searchForm.huodongbiaoti" placeholder="活动标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('jianglihuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('jianglihuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0 0 2px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","borderColor":"rgba(252,234,238,1)","margin":"0 0 20px","borderRadius":"3px","background":"rgba(255,255,255,1)","borderWidth":"0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jianglihuodong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan12" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('jianglihuodong','参与')" @click="jifenduihuanCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								参与
							</el-button>
							<el-button class="edit" v-if=" isAuth('jianglihuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								更改
							</el-button>

							<el-button class="view" v-if="isAuth('jianglihuodong','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan12" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jianglihuodong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<jifenduihuan-cross-add-or-update v-if="jifenduihuanCrossAddOrUpdateFlag" :parent="this" ref="jifenduihuanCrossaddOrUpdate"></jifenduihuan-cross-add-or-update>
...
import jifenduihuanCrossAddOrUpdate from "../jifenduihuan/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				jifenduihuanCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			jifenduihuanCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jifenduihuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jianglihuodong');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jifenduihuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jifenduihuanCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.huodongbianhao!='' && this.searchForm.huodongbianhao!=undefined){
            params['huodongbianhao'] = '%' + this.searchForm.huodongbianhao + '%'
          }
           if(this.searchForm.huodongbiaoti!='' && this.searchForm.huodongbiaoti!=undefined){
            params['huodongbiaoti'] = '%' + this.searchForm.huodongbiaoti + '%'
          }
      this.$http({
        url: "jianglihuodong/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.$router.push({path:'/discussjianglihuodong',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	jianglihuodongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jianglihuodong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
