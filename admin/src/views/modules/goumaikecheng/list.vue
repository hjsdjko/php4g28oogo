<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">课程编号</label>
						<el-input v-model="searchForm.kechengbianhao" placeholder="课程编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">课程分类</label>
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('goumaikecheng','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('goumaikecheng','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('goumaikecheng','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						批量支付
					</el-button>


					<el-button class="btn18" v-if="isAuth('goumaikecheng','课程销售额统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('goumaikecheng','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('goumaikecheng','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan12" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('goumaikecheng','学习')" @click="xuexikechengCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								学习
							</el-button>
							<el-button class="edit" v-if=" isAuth('goumaikecheng','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('goumaikecheng','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<xuexikecheng-cross-add-or-update v-if="xuexikechengCrossAddOrUpdateFlag" :parent="this" ref="xuexikechengCrossaddOrUpdate"></xuexikecheng-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kechengfeiyongChart1" style="width:100%;height:600px;"></div>
...
import xuexikechengCrossAddOrUpdate from "../xuexikecheng/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				xuexikechengCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			xuexikechengCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.xuexikechengCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','goumaikecheng');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xuexikechengCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xuexikechengCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','goumaikecheng');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var kechengfeiyongChart1 = echarts.init(document.getElementById("kechengfeiyongChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                kechengfeiyongChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kechengfeiyongChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.kechengbianhao!='' && this.searchForm.kechengbianhao!=undefined){
            params['kechengbianhao'] = '%' + this.searchForm.kechengbianhao + '%'
          }
	...
	goumaikechengstatusChange(e,row){
		if(row.status==0){
