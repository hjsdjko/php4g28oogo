<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="课程分类" prop="kechengfenlei">
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					...
					<el-button class="btn18" v-if="isAuth('kechengxinxi','课程统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="kechengfengmian" width="200" label="课程封面">
						<template slot-scope="scope">
							<div v-if="scope.row.kechengfengmian">
								<img v-if="scope.row.kechengfengmian.substring(0,4)=='http'" :src="scope.row.kechengfengmian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.kechengfengmian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="kechengfujian" label="课程附件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.kechengfujian" type="text" size="small" @click="download(scope.row.kechengfujian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="kechengbiao" label="课程表">
						<template slot-scope="scope">
							<el-button v-if="scope.row.kechengbiao" type="text" size="small" @click="download(scope.row.kechengbiao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','购买')" @click="goumaikechengCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
		...
		<goumaikecheng-cross-add-or-update v-if="goumaikechengCrossAddOrUpdateFlag" :parent="this" ref="goumaikechengCrossaddOrUpdate"></goumaikecheng-cross-add-or-update>
...
import goumaikechengCrossAddOrUpdate from "../goumaikecheng/add-or-update";
	export default {
		data() {
			return {
				...
				goumaikechengCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			goumaikechengCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.goumaikechengCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.goumaikechengCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.goumaikechengCrossaddOrUpdate.init(row.id,type);
				});
			},
